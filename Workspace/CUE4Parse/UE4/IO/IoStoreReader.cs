        ...
        private byte[] Read(long offset, long length)
        {
            ...
            for (int blockIndex = firstBlockIndex; blockIndex <= lastBlockIndex; blockIndex++)
            {
                ...
                else
                {
                    ...
                    if (compressionMethod == Compression.CompressionMethod.Oodle)
                        src = Galaxy_Swapper_v2.Workspace.Swapping.Compression.Compression.Decompress(compressedBuffer, (int)uncompressedSize, compressionMethod);
                    else
                    {
                        Compression.Compression.Decompress(compressedBuffer, 0, (int)rawSize, uncompressedBuffer, 0, (int)uncompressedSize, compressionMethod, reader);
